...
export class userModel {
  ...
  static async getAllEmails() {
    const [emails] = await connection.query(`SELECT CorreoEmail FROM usuario`);
    return emails;
  }
  ...
  static async delete({ id }) {
    try {
      await connection.query("DELETE FROM usuario WHERE CedulaCarnet = ?", [
        id,
      ]);
    } catch (error) {
  ...
  static async deleteOldUnverified({ maxAgeDays = 7 }) {
    try {
      ...
      console.error(
        "Error eliminando usuarios no verificados antiguos:",
        error,
      );
      throw error;
    }
